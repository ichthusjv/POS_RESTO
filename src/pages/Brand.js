...
function Brand() {
  ...
  const openUpdateBrandForm = (data) => {
    setSelectedData(data);
    setShowUpdateBrandForm(true);
  }
  const closeUpdateBrandForm = () => {
    setShowUpdateBrandForm(false);
    resetParameters();
  }

  useEffect(() => {
    if(showUpdateBrandForm){
      setDescription(selectedData.description);
      setIsActive(selectedData.isActive);
    }
  }, [showUpdateBrandForm, selectedData])

  //Delete Brand
  const [showDeleteBrandForm, setShowDeleteBrandForm] = useState(false);
  const openDeleteBrandForm = (data) => {
    setSelectedData(data);
    setShowDeleteBrandForm(true);
  }
  const closeDeleteBrandForm = () => {
    setShowDeleteBrandForm(false);
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-1 lg:mr-32 my-3">
          ...
          <div>
            <Button onClick={openAddBrandForm}>Add Brand</Button>
          </div>
      </div>
              ...
              <TableRow key={i}>
                ...
                <TableCell className="text-center" style={{justifyItems: "center"}}>
                  <div className="flex items-center space-x-4">
                    <Button layout="link" size="icon" aria-label="Edit" onClick={()=>openUpdateBrandForm(data)}>
                      <EditIcon className="w-5 h-5" aria-hidden="true" />
                    </Button>
                    <div className="hidden">
                      <Button layout="link" size="icon" aria-label="Delete" onClick={()=>openDeleteBrandForm(data)}>
      ...
      {/* Add Brand Form */}
      <Modal id="modal" isOpen={showAddBrandForm}>
        <ModalHeader>Add Brand Form</ModalHeader>
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeAddBrandForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
      ...
      {/* Update Brand Form */}
      <Modal id="modal" isOpen={showUpdateBrandForm}>
        <ModalHeader>Add Brand Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
          <Label className="my-5">
            <span>Description</span>
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeUpdateBrandForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
      ...
      {/* Delete Brand Form */}
      <Modal id="modal" isOpen={showDeleteBrandForm}>
        <ModalHeader>Delete Brand Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
            <span>{`Are you sure you want to delete ${selectedData.description} ?`}</span>
        </ModalBody>
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeDeleteBrandForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleDeleteBrand}>Delete Brand</Button>
          </div>
        </ModalFooter>
      </Modal>
