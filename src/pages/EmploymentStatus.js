...
function EmploymentStatus() {
  ...
  const apiUrl =  domain + "/api/EmploymentStatuses";
  ...
  const [showAddEmploymentStatusForm, setShowAddEmploymentStatusForm] = useState(false);
  const openAddEmploymentStatusForm = () => {
    setShowAddEmploymentStatusForm(true);
  }
  const closeAddEmploymentStatusForm = () => {
    setShowAddEmploymentStatusForm(false);
    resetParameters()
  }

  // Update EmploymentStatus
  const [showUpdateEmploymentStatusForm, setShowUpdateEmploymentStatusForm] = useState(false);
  const openUpdateEmploymentStatusForm = (data) => {
    setSelectedData(data);
    setShowUpdateEmploymentStatusForm(true);
  }
  const closeUpdateEmploymentStatusForm = () => {
    setShowUpdateEmploymentStatusForm(false);
    resetParameters();
  }

  //Delete EmploymentStatus
  const [showDeleteEmploymentStatusForm, setShowDeleteEmploymentStatusForm] = useState(false);
  const openDeleteEmploymentStatusForm = (data) => {
    setSelectedData(data);
    setShowDeleteEmploymentStatusForm(true);
  }
  const closeDeleteEmploymentStatusForm = () => {
    setShowDeleteEmploymentStatusForm(false);
  }

  useEffect(() => {
    if(showUpdateEmploymentStatusForm){
      setDescription(selectedData.description);
      setIsActive(selectedData.isActive);
    }
  }, [showUpdateEmploymentStatusForm, selectedData])

  const [description, setDescription] = useState("");
  const [isActive, setIsActive] = useState("");

  const handleAddEmploymentStatus = async (event) => {
    ...
    try {
        ...
        setLoading(false);
        resetParameters();
        closeAddEmploymentStatusForm();
    }
  };
  const handleUpdateEmploymentStatus = async (event) => {
    event.preventDefault();
    setLoading(true);
    ...
    try {
        ...
        setLoading(false);
        resetParameters();
        closeUpdateEmploymentStatusForm();
    }
  };
  const handleDeleteEmploymentStatus = async () => {
    ...
    try {
        ...
        setLoading(false);
        resetParameters();
        closeDeleteEmploymentStatusForm();
    }
  };
  ...
  return (
    <>
      <PageTitle>Employment Statuses</PageTitle>
      {/* <!-- Search input --> */}
      <div className="flex flex-1 lg:mr-32 my-3">
          ...
          <div>
            <Button onClick={openAddEmploymentStatusForm}>Add Employment Status</Button>
          </div>
      </div>
              ...
              <TableRow key={i}>
                ...
                <TableCell className="text-center" style={{justifyItems: "center"}}>
                  <div className="flex items-center space-x-4">
                    <Button layout="link" size="icon" aria-label="Edit" onClick={()=>openUpdateEmploymentStatusForm(data)}>
                      <EditIcon className="w-5 h-5" aria-hidden="true" />
                    </Button>
                    <div className='hidden'>
                      <Button layout="link" size="icon" aria-label="Delete" onClick={()=>openDeleteEmploymentStatusForm(data)}>
      ...
      <Modal id="modal" isOpen={showAddEmploymentStatusForm}>
        <ModalHeader>Add Employment Status Form</ModalHeader>
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeAddEmploymentStatusForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleAddEmploymentStatus}>Add Employment Status</Button>
          </div>
        </ModalFooter>
      </Modal>

      {/* Update Employment Status Form */}
      <Modal id="modal" isOpen={showUpdateEmploymentStatusForm}>
        <ModalHeader>Add Employment Status Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
          <Label className="my-5">
            <span>Description</span>
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeUpdateEmploymentStatusForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleUpdateEmploymentStatus}>Update Employment Status</Button>
          </div>
        </ModalFooter>
      </Modal>

      {/* Delete Employment Status Form */}
      <Modal id="modal" isOpen={showDeleteEmploymentStatusForm}>
        <ModalHeader>Delete Employment Status Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
            <span>{`Are you sure you want to delete ${selectedData.description} ?`}</span>
        </ModalBody>
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeDeleteEmploymentStatusForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleDeleteEmploymentStatus}>Delete Employment Status</Button>
          </div>
        </ModalFooter>
      </Modal>
