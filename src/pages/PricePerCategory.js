...
function PricePerCategory() {
  ...
  // Add PricePerCategory
  const [showAddPricePerCategoryForm, setShowAddPricePerCategoryForm] = useState(false);
  const openAddPricePerCategoryForm = () => {
    setShowAddPricePerCategoryForm(true);
  }
  const closeAddPricePerCategoryForm = () => {
    setShowAddPricePerCategoryForm(false);
    resetParameters()
  }

  // Update PricePerCategory
  const [showUpdatePricePerCategoryForm, setShowUpdatePricePerCategoryForm] = useState(false);
  const openUpdatePricePerCategoryForm = (data) => {
    setSelectedData(data);
    setShowUpdatePricePerCategoryForm(true);
  }
  const closeUpdatePricePerCategoryForm = () => {
    setShowUpdatePricePerCategoryForm(false);
    resetParameters();
  }

  //Delete PricePerCategory
  const [showDeletePricePerCategoryForm, setShowDeletePricePerCategoryForm] = useState(false);
  const openDeletePricePerCategoryForm = (data) => {
    setSelectedData(data);
    setShowDeletePricePerCategoryForm(true);
  }
  const closeDeletePricePerCategoryForm = () => {
    setShowDeletePricePerCategoryForm(false);
  }

  useEffect(() => {
    if(showUpdatePricePerCategoryForm){
      setDescription(selectedData.description);
      setCategoryId(selectedData.categoryId);
      setCategoryDescription(selectedData.categoryDescription);
  ...
  }, [showUpdatePricePerCategoryForm, selectedData])
  ...
  const handleAddPricePerCategory = async (event) => {
    ...
    try {
        ...
        setLoading(false);
        resetParameters();
        closeAddPricePerCategoryForm();
    }
  };
  const handleUpdatePricePerCategory = async (event) => {
    event.preventDefault();
    setLoading(true);
    ...
    try {
        ...
        setLoading(false);
        resetParameters();
        closeUpdatePricePerCategoryForm();
    }
  };
  const handleDeletePricePerCategory = async () => {
    setLoading(true);
    try {
        ...
        setLoading(false);
        resetParameters();
        closeDeletePricePerCategoryForm();
    }
  };
  ...
  return (
    <>
      <PageTitle>Price Per Category</PageTitle>
      {/* <!-- Search input --> */}
      <div className="flex flex-1 lg:mr-32 my-3">
          ...
          <div>
            <Button onClick={openAddPricePerCategoryForm}>Add Pricing</Button>
          </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <tr>
              ...
              <TableCell className="text-center" onClick={() => handleSort('categoryDescription')}>
                Category
                {sortState.id === 'categoryDescription' && (
                  <span>{sortState.direction === 'asc' ? '↑' : sortState.direction === 'desc' ? '↓' : ''}</span>
                )}
              </TableCell>
              <TableCell className="text-center" onClick={() => handleSort('description')}>
          ...
          <TableBody>
            {dataTablePerPage.map((data, i) => (
              <TableRow key={i}>
                ...
                <TableCell className="text-center" style={{justifyItems: "center"}}>
                  <span className="text-sm">{data.categoryDescription}</span>
                </TableCell>
                ...
                <TableCell className="text-center" style={{justifyItems: "center"}}>
                  <div className="flex items-center space-x-4">
                    <Button layout="link" size="icon" aria-label="Edit" onClick={()=>openUpdatePricePerCategoryForm(data)}>
                      <EditIcon className="w-5 h-5" aria-hidden="true" />
                    </Button>
                    <Button layout="link" size="icon" aria-label="Delete" onClick={()=>openDeletePricePerCategoryForm(data)} className="hidden">
      ...
      {/* Add PricePerCategory Form */}
      <Modal id="modal" isOpen={showAddPricePerCategoryForm}>
        <ModalHeader>Add Price Per Category Form</ModalHeader>
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeAddPricePerCategoryForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleAddPricePerCategory}>Add PricePerCategory</Button>
          </div>
        </ModalFooter>
      </Modal>

      {/* Update PricePerCategory Form */}
      <Modal id="modal" isOpen={showUpdatePricePerCategoryForm}>
        <ModalHeader>Update Price Per Category Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
          <Label className="my-5">
            <span>Description</span>
            <Input className="mt-1" placeholder="Description" value={description} onChange={(e)=>setDescription(e.target.value)} />
        ...
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeUpdatePricePerCategoryForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleUpdatePricePerCategory}>Update PricePerCategory</Button>
          </div>
        </ModalFooter>
      </Modal>

      {/* Delete PricePerCategory Form */}
      <Modal id="modal" isOpen={showDeletePricePerCategoryForm}>
        <ModalHeader>Delete Price Per Category Form</ModalHeader>
        <ModalBody className="max-h-xl overflow-y-auto p-2">
            <span>{`Are you sure you want to delete ${selectedData.description} ?`}</span>
        </ModalBody>
        <ModalFooter>
          <div className="block">
            <Button layout="outline" onClick={closeDeletePricePerCategoryForm}>
              Cancel
            </Button>
          </div>
          <div className="block">
            <Button onClick={handleDeletePricePerCategory}>Delete PricePerCategory</Button>
          </div>
        </ModalFooter>
      </Modal>
...
export default PricePerCategory
