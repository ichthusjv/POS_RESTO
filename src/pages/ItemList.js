        ...
        <Table>
          <TableHeader>
            <tr>
              ...
              <TableCell className="text-center" onClick={() => handleSort('name')}>
                Name
                {sortState.id === 'name' && (
                  <span>{sortState.direction === 'asc' ? '↑' : sortState.direction === 'desc' ? '↓' : ''}</span>
                )}
              </TableCell>
              <TableCell className="text-center" onClick={() => handleSort('stock')}>
                Stock
                {sortState.id === 'stock' && (
                  <span>{sortState.direction === 'asc' ? '↑' : sortState.direction === 'desc' ? '↓' : ''}</span>
                )}
              ...
              <TableRow key={i}>
                ...
                <TableCell className="text-center" style={{justifyItems: "center"}}>
                  <div className="flex items-center text-sm">
                    <div>
                      <p className="font-semibold truncate w-64">{data.name}</p>
                    </div>
                  </div>
                </TableCell>
      ...
      <Modal id="modal" isOpen={showAddItemForm}>
        ...
        <ModalBody className="max-h-xl overflow-y-auto p-2">
          <Label className="my-5">
            <span>Name</span>
            <Input className="mt-1" placeholder="Product" value={name} onChange={(e)=>setName(e.target.value)} />
            {!name && <span className='text-xs text-red-500'>required</span>}
          </Label>
          
          <Label className="my-5">
            <span>Stock</span>
            <Input className="mt-1" placeholder="Stock" type="number" value={stock} onChange={(e)=>setStock(e.target.value)} />
            {!stock && <span className='text-xs text-red-500'>required</span>}
          </Label>
        </ModalBody>
        <ModalFooter>
          <div className="block">
